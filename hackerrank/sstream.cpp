...
vector<int> parseIntegers(string stringName)
{
    vector<int> v;    // Declares a vector to store the integers
    ...
    {
        ss>>temp>>ch;   // Extract the comma seperated integers
        v.push_back(temp);   // Push the integer onto the vector array
    }
    return v;
}
...
int main()
{
    string str;
    cin >> str;
    for(const int& i : parseIntegers(str))
        cout << i << endl;
    return 0;
}
